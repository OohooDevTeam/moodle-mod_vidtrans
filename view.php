...
if ($id) {
    ...
    $course = $DB->get_record('course', array('id' => $vidtrans->course), '*', MUST_EXIST);
    $cm = get_coursemodule_from_instance('vidtrans', $vidtrans->id, $course->id, false, MUST_EXIST);
} else {
...
add_to_log($course->id, 'vidtrans', 'view', "view.php?id={$cm->id}", $vidtrans->name, $cm->id);
...
//$PAGE->add_body_class('vidtrans-'.$somevar);
// Output starts here
echo $OUTPUT->header();
